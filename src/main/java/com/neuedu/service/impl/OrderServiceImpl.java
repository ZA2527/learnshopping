...
public class OrderServiceImpl implements IOrderService{
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        //步骤三：判断订单状态并取消
        if(order.getStatus()!=ResponseCode.OrderStatusEnum.ORDER_UN_PAY.getCode()){
            return ServerResponse.createServerResponseByError("订单不可取消");
        }
        //步骤四：返回结果
        order.setStatus(ResponseCode.OrderStatusEnum.ORDER_CANCELED.getCode());
...
//        orderVO.setOrderItemVOList();
        ...
        ResponseCode.OrderStatusEnum orderStatusEnum = ResponseCode.OrderStatusEnum.codeOf(order.getStatus());
        ...
        ResponseCode.PaymentEnum paymentEnum = ResponseCode.PaymentEnum.codeOf(order.getPaymentType());
        if(paymentEnum!=null){
    ...
    private Order createOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
        ...
        order.setStatus(ResponseCode.OrderStatusEnum.ORDER_UN_PAY.getCode());
        //订单金额
        order.setPayment(orderTotalPrice);
        order.setPostage(0);
        order.setPaymentType(ResponseCode.PaymentEnum.ONLINE.getCode());
        //保存订单
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cart:cartList){
            ...
            if(product.getStatus() != ResponseCode.ProductStatusEnum.PRODUCT_ONLINE.getCode()){//商品下架
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        if(order.getStatus()>=ResponseCode.OrderStatusEnum.ORDER_PAYED.getCode()){
            //防止支付宝重复回调
        ...
        if(trade_status.equals(Const.TRADE_SUCCESS)){
            //支付成功
            //更改订单的状态，更改支付时间
            order.setStatus(ResponseCode.OrderStatusEnum.ORDER_PAYED.getCode());
        ...
        payInfo.setPayPlatform(ResponseCode.PaymentPlatformEnum.ALIPAY.getCode());
    ...
    public ServerResponse pay(Order order) {
        ...
        String subject = "订单:" + order.getOrderNo() + "门店当面付扫码消费" + order.getPayment().intValue();
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://t2h42v.natappfree.cc/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
